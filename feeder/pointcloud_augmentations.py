...
def joint_courruption(input_data):
    ...
    if flip_prob < 0.5:
        ...
        joint_indicies = np.random.choice(V, int(0.25*V), replace=False)
        out[:, :, joint_indicies] = 0
        return out
    else:
        ''' 随机选择15个关节进行随机空间变换'''
        # joint_indicies = np.random.choice(25, random.randint(5, 10), replace=False)
        joint_indicies = np.random.choice(V, int(0.75*V), replace=False)

        temp = out[:, :, joint_indicies]
        # random.uniform()服从均匀分布
        Corruption = np.array([
            [random.uniform(-1, 1), random.uniform(-1, 1), random.uniform(-1, 1)],
            [random.uniform(-1, 1), random.uniform(-1, 1), random.uniform(-1, 1)],
            [random.uniform(-1, 1), random.uniform(-1, 1), random.uniform(-1, 1)]])
        temp = np.dot(temp.transpose([1, 2, 0]), Corruption)
...
def pose_augmentation(input_data):
    """ 随机生成空间变换矩阵(对角元素为1--坐标轴没有缩放)对整体骨架做变换 """
    Shear = np.array([
    ...
    result = np.dot(temp_data.transpose([1, 2, 0]), Shear.transpose())  # CTV to TVC
    output = result.transpose(2, 0, 1)

    return output


def temporal_cropresize(input_data, num_of_frames, l_ratio, output_size):
    """在时间序列上随机选取连续的帧并用双线性插值补全到输出维度"""
    ...
    temporal_context = temporal_context.permute(0, 2, 1).contiguous().view(C * V, temporal_crop_length)
    temporal_context = temporal_context[None, :, :, None]  # [1 C*V*M temporal_crop_length 1]
    # 双线性插值到output_size
    ...
    temporal_context = temporal_context.contiguous().view(C, V, output_size).permute(0, 2, 1).contiguous().numpy()
