...
def read_skeleton_filter(file):
    ...
    skeleton_sequence:
        ...
        ----frame_info_1
            --numBody(1 or 2)
            --bodyInfo
              --body_info_key(10 items)
              --numJoint(25)
              --JointInfo(12 items * n)
        ----...
        ----frame_info_n
    """
...
def get_nonzero_std(s):  # tvc
    ...
    s = s[index]  # 去除零矩阵骨架帧
    if len(s) != 0:
        s = s[:, :, 0].std() + s[:, :, 1].std() + s[:, :, 2].std()  # three channels - x,y,z三个方向标准差的和
    else:
        s = 0
    return s
...
def read_xyz(file, max_body=4, num_joint=25):  # 取了前两个body
    ...
    index = energy.argsort()[::-1][0:max_body_true]  # 从大到小排列
    data = data[index]

    data = data.transpose(3, 1, 2, 0)  # [xyz, frame_num, joints_num, body_ID]
    return data
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path',
                        default='D:/study/prcd/Skeleton-based Action Recognition/code/nturgbd_raw/nturgb+d_skeletons')
    parser.add_argument('--ignored_sample_path',
                        default='../data/samples_with_missing_skeletons.txt')
    parser.add_argument('--out_folder', default='../data/NTU-RGB-D-60-AGCN/')
    benchmark = ['xsub', 'xview']  # 分别代表按人物ID划分数据集和按视角（共3个视角）划分数据集
