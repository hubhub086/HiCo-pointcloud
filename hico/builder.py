...
from .hi_encoder_point import PretrainingEncoder_point
...
class HiCo(nn.Module):
    def __init__(self, args_encoder, dim=3072, K=65536, m=0.999, T=0.07, pre_dataset='ntu60'):
        """
        ...
        if pre_dataset == 'pointcloud':
            self.encoder_q = PretrainingEncoder_point(**args_encoder)
            self.encoder_k = PretrainingEncoder_point(**args_encoder)
        else:
    ...
    def forward(self, qc_input, qp_input, kc_input, kp_input):
        ...
        l_pos_c = torch.einsum('nc,nlc->nl', [qc0, kc])  # (N,G)
